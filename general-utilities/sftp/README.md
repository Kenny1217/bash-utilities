...
### 1. `sftp-get-transfer.sh`

This script downloads a file from a remote server to a local directory.

#### Usage:
```
bash ./sftp-get-transfer.sh <SFTP_USER> <SFTP_HOST> <REMOTE_DIRECTORY> <REMOTE_FILENAME> <LOCAL_DIRECTORY> <LOCAL_FILENAME> 
```

### 2. `sftp-put-transfer.sh`

This script uploads a file from a local directory to a remote server.

#### Usage:
```
bash ./sftp-get-transfer.sh <SFTP_USER> <SFTP_HOST> <LOCAL_DIRECTORY> <LOCAL_FILENAME> <REMOTE_DIRECTORY> <REMOTE_FILENAME> 
```